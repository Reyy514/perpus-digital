        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-base-100 overflow-hidden shadow-xl rounded-xl">
                <div class="p-6 md:p-8">
                    <form action="{{ route('admin.books.update', $book) }}" method="POST" enctype="multipart/form-data" x-data="{ imagePreview: '{{ $book->cover_image_url ? asset('storage/' . $book->cover_image_url) : null }}' }">
                        @csrf
                        @method('PATCH')
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <!-- Mengambil nama penulis dari relasi: $book->author->name -->
                                <x-text-input id="author" class="block mt-1 w-full" type="text" name="author" :value="old('author', $book->author->name)" required />
                            ...
                            <div class="md:col-span-2 space-y-6">
                                ...
                                <div>
                                    <x-input-label for="cover_image" value="Ganti Sampul Buku (Opsional)" />
                                ...
                                <div>
                                    ...
                                    <textarea name="description" id="description" rows="5" class="mt-1 block w-full rounded-lg border-base-300 bg-base-200/50 text-sm focus:border-primary focus:ring-primary/40 focus:ring-2">{{ old('description', $book->description) }}</textarea>
                        ...
                        <div class="flex items-center justify-end mt-8 border-t border-base-300 pt-6 gap-4">
                            ...
                            <x-primary-button>Perbarui Buku</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
